"""Latency-vs-decay test: backtest win rate (z>=0.45) PER DAY on the recorder,
for the exact days the bot traded live. If recent days still show ~77%, the
live 57% is EXECUTION (latency, recoverable). If recent days also show ~57%,
the edge has DECAYED (not recoverable by tuning).
"""
import io, math, os
...
FEE = 0.07; DAYS = [f"2026-06-{d:02d}" for d in range(22, 31)]  # recent window incl live days
...
# load per day, compute win rate, keyed by day
B = {}
...
print(f"{'day':<12} {'entries':>7} {'win%':>6} {'EV/$1':>7}")
for day in DAYS:
    tokmap = {}
    ...
    for tid, sm_ in book.items():
        ...
        for i in range(len(grid)):
            ...
            A = ba[i + 1] if i + 1 < len(grid) else ba[i]
            if not np.isfinite(A) or not (0.30 <= A <= 0.97): continue
            z = disp[i] / (vv * math.sqrt(t))
            if z < ZMIN: continue
            if pcal(z) - A - FEE * A * (1 - A) < EDGE_MIN: continue
            sh = 1.0 / A; net1 = (sh - 1.0 - sh * FEE * A * (1 - A)) if win else -1.0
            R.append((win, net1)); break
    if R:
        wr = np.mean([w for w, _ in R]); ev = np.mean([n for _, n in R])
        print(f"{day:<12} {len(R):>7} {wr:>5.0%} {ev:>+7.3f}")
